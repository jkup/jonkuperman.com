---
title: 'Compiler Resources'
date: '2020-10-26T22:12:13.284Z'
tags: ['Compilers']
---

This is mostly a list for myself, but I thought it might be fun to share. I want to learn a lot more about parsers and compilers. Here is my current "To Learn" list with courses, videos and books (in no particular order)!

-   [Compilers: Principles, Techniques, and Tools](https://www.amazon.com/Compilers-Principles-Techniques-Tools-2nd/dp/0321486811)
-   [Language Implementation Patterns](https://www.amazon.com/Language-Implementation-Patterns-Domain-Specific-Programming/dp/193435645X)
-   [Writing An Interpreter in Go](https://interpreterbook.com/)
-   [Writing A Compiler In Go](https://compilerbook.com/)
-   [Crafting Interpreters](http://craftinginterpreters.com/)
-   [Game Programming Patterns](http://gameprogrammingpatterns.com/)
-   [Parsing Algorithms](https://www.dmitrysoshnikov.education/p/essentials-of-parsing)
-   [A crash course in compilers](https://increment.com/programming-languages/crash-course-in-compilers/)
-   [Just write the #!%/\* parser](https://tiarkrompf.github.io/notes/?/just-write-the-parser/)
-   [Compiler Compiler: A Twitch series about working on a JavaScript engine](https://hacks.mozilla.org/2020/06/compiler-compiler-working-on-a-javascript-engine/)
-   [Why JavaScript syntax is hard to parse](https://github.com/mozilla-spidermonkey/jsparagus/blob/master/js-quirks.md#readme)
-   [Essentials of Interpretation](http://dmitrysoshnikov.com/courses/essentials-of-interpretation/)
